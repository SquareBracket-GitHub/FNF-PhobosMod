...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var redSky:FlxSprite = new FlxSprite();
		...
		buildings.loadGraphic(Paths.image('buildings'));
		...
		shadow.loadGraphic(Paths.image('shadow'));
		shadow.setGraphicSize(Std.int(shadow.width * 1.05));
		shadow.updateHitbox();
		shadow.screenCenter();
		add(shadow);

		logoBl = new FlxSprite(titleJSON.titlex, titleJSON.titley);
		logoBl.frames = Paths.getSparrowAtlas('PhobosLogo');
		logoBl.screenCenter();
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxG.sound.play(Paths.sound('confirmMenu1'));
